...
public class Coleccion {
    public static void main(String[] args) {
        ...
        long cantMayorCinco = cursos.stream().filter(curso -> curso.getDuracion() > 5).count();

        //Obtener la cantidad de cursos con una duración menor a 2 horas.
        long cantMenorDos = cursos.stream().filter(curso -> curso.getDuracion() < 2).count();

        //Listar el título de todos aquellos cursos con una cantidad de vídeos mayor a 50.
        List<Curso> list = cursos.stream().filter(curso -> curso.getVideos() > 50).toList();
        ...
        System.out.println("Duracion de todos los cursos " + cursos.stream().mapToDouble(Curso::getDuracion).sum());
        ...
        Curso cursoMayorDuracion = cursos.stream()
                .max(Comparator.comparingDouble(Curso::getDuracion))
                .orElse(null);

        //Crear una lista de Strings con todos los titulos de los cursos.
        List<String> listaTitulos = cursos.stream().map(Curso::getTitulo).collect(Collectors.toList());
